...
function camp_hide(radiovalue,element)
{
	...
	else
	{
		...
		{
			if ('true' == '<?php
				if (in_array('perday',(array) $sa_params->get('pricing_options', array())))
				{
					echo 'true';
				}
				else
			...
			'<option value="perday" '+day_sel+' ><?php echo Text::_('COM_SOCIALADS_FORM_LBL_ZONE_PER_DAY') ?></option>'
			);
			jQuery("select").trigger("liszt:updated");
...
function migrateads()
{
	var migrate_sure=confirm('<?php echo Text::_('COM_SOCIALADS_PRICING_MIGRATE_ADS_CONFORMATION'); ?>');

	if (migrate_sure==true)
	{
		var camp_or_old = techjoomla.jQuery('.migrate').val();
		...
		techjoomla.jQuery.ajax({
			...
			success: function(data)
			{
				if(data == 1)
				{
					...
					alert('<?php echo Text::_('COM_SOCIALADS_MIGRATE_CONFIRM_SAVE_POP'); ?>');
...
class JFormFieldPaymentmode extends JFormField
{
	...
	public function getInput()
	{
		...
		$options = array(
			HTMLHelper::_('select.option', 'wallet_mode', Text::_('COM_SOCIALADS_PRICING_ADVERTISING_WALLET')) ,
			HTMLHelper::_('select.option', 'pay_per_ad_mode', Text::_('COM_SOCIALADS_PRICING_PAY_PER_DAY'))
		);
		$return = HTMLHelper::_('select.genericlist', $options, 'jform[payment_mode]',
		'class="inputbox form-select migrate" onclick="camp_hide(this.value,\'.camp_price\');"', 'value', 'text', $selected, '');
		...
		$return .= ' >
				<div id="migrate_btn">
					<div class="alert alert-error">' . Text::_('COM_SOCIALADS_PRICING_MIGRATE_NOTICE_FOR_WALLET') . '</div>
					<input type="button" class="btn btn-danger"  width="50%" onclick="migrateads()" value="'
					. Text::_('COM_SOCIALADS_PRICING_MIGRATE_AD_WALLET_MODE') . '" />
				</div>
				<div id="migration_status" style="display:none" class="col-md-12 mt-4">
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="' . Uri::root() . '/media/com_sa/images/publish.png' . '" > '
						. Text::sprintf('COM_SOCIALADS_PRICING_WALLET_STEP1', $camp_currency_daily, $currency) . '</span>
					</div>
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="' . Uri::root() . '/media/com_sa/images/publish.png' . '" > '
						. Text::sprintf('COM_SOCIALADS_PRICING_WALLET_STEP2', $currency) . '</span>
					</div>
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="' . Uri::root() . '/media/com_sa/images/publish.png' . '" > '
						. Text::_('COM_SOCIALADS_PRICING_WALLET_STEP3') . '</span>
					</div>
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="' . Uri::root()
						. 'media/com_sa/images/publish.png' . '" > '
						. Text::_('COM_SOCIALADS_PRICING_WALLET_STEP4') . '</span>
					</div>
				</div>
			</div>
			<div id="loader_image_div" style="display:none">
				 <div class="alert alert-warning">' . Text::_('COM_SOCIALADS_PRICING_PLEASE_WAIT') . ' </div>
				 <img src="' . Uri::root() . 'media/com_sa/images/loader_light_blue.gif" width="128" height="15" border="0" />
			</div>
		...
		$return .= '
				<div id="migrate_btn_for_old">
					<div class="alert alert-error">' . Text::_('COM_SOCIALADS_PRICING_MIGRATE_NOTICE_FOR_PAY_PER_AD') . '</div>
					<input type="button" class="btn btn-danger"  width="50%" onclick="migrateads()" value="' .
					Text::_('COM_SOCIALADS_PRICING_MIGRATE_PAY_PER_AD_MODE') . '" />
				</div>

				<div id="migration_status_for_old" style="display:none" class="col-md-12 mt-4">
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="'
						. Uri::root() . 'media/com_sa/images/publish.png' . '" > '
						. Text::_('COM_SOCIALADS_PRICING_PAY_PER_AD_STEP1') . '</span>
					</div>
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="' . Uri::root()
						. 'media/com_sa/images/publish.png' . '" > '
						. Text::_('COM_SOCIALADS_PRICING_PAY_PER_AD_STEP2') . '</span>
					</div>
					<div class="completed_old_migrate row" >
						<span class="image_span alert alert-info" ><img class="image"  src="'
						. Uri::root() . 'media/com_sa/images/publish.png' . '" > '
						. Text::_('COM_SOCIALADS_PRICING_PAY_PER_AD_STEP3') . '</span>
					</div>
				</div>
			</div>
				<div id="migrate_error_div" class="alert alert-warning" style="display:none"  >' .
				Text::_('COM_SOCIALADS_ADS_MIGRATION_ERROR') . '</div>
		</div>
		';
