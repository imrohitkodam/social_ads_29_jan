...
function ajaxUpload(form, url_action, id_element, html_show_loading, html_error_http) {
	...
	form.setAttribute("success", onAfterSuccessUpload());
	...
	function onAfterSuccessUpload () {
		setTimeout(function() {
			if (jQuery('#upload_area .errorPresent').length)
			{
				jQuery('#ad_image').val('');
				alert(Joomla.JText._('COM_SOCIALADS_ERR_SOMETHING_WENT_WRONG'));
			}
		}, 2500);
	};
}
...
function html5FileCheck() {
	...
	if(window.File && window.FileReader && window.FileList && window.Blob) {
		...
		var allowedType = saAllowedMimeTypes;
		var adType = techjoomla.jQuery('.sa-form #adtype').val();
		if (adType == 'html5_zip')
		{
			allowedType = ['application/zip', 'application/x-zip-compressed'];
		}
		/*Check mime type*/
		if(techjoomla.jQuery.inArray(ftype, allowedType) == -1) {
