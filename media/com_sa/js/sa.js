...
var sa = {
	...
	checkForZeroAndAlpha: function (ele, allowedChar, msg) {
		if (ele.value && ele.value != 0) {
			...
			techjoomla.jQuery('#html-container').hide();
			techjoomla.jQuery('input[name ="payment_gateway"]')[0].checked = false;
			Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_PAYMENT_MIN_AMT_SHOULD_GREATER_MSG')] });
			ele.value = '';
		}
	},
		...
		for (i = 0; i < el.value.length; i++) {
			if ((el.value.charCodeAt(i) <= 47 || el.value.charCodeAt(i) >= 58) || (el.value.charCodeAt(i) == 45)) {
				...
				if (allowed_ascii == el.value.charCodeAt(i)) {
					...
					Joomla.renderMessages({ 'error': [enter_numerics] });
	...
	validateCoupon: function (e) {
		if (techjoomla.jQuery('#sa_coupon_chk').is(':checked') && techjoomla.jQuery('#sa_coupon_code').val() == '') {
	...
	create: {
		...
		initStepsJs: function () {
			techjoomla.jQuery(document).ready(function () {
				techjoomla.jQuery('#btnWizardNext').on('click', function () {
					techjoomla.jQuery('#system-message-container').hide();
				...
				techjoomla.jQuery('#MyWizard').on('change', function (e, data) {
					...
					if (data.direction === 'next') {
						...
						techjoomla.jQuery.ajax({
							url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.autoSave&stepId=' + stepId + '&tmpl=component',
							...
							success: function (response) {
								...
								if (stepId == 'ad-design' && data.direction === 'next') {
									if (techjoomla.jQuery('#sa_accpt_terms').length === 1 && techjoomla.jQuery('#sa_accpt_terms').is(':checked') === false) {
										alert(Joomla.JText._('COM_SOCIALADS_ACCEPT_TERMS'));
										return e.preventDefault();
									}
									...
									if ((altadbutton == 1) || (affiliate == 1)) {
										window.location.assign(Joomla.getOptions('system.paths').base + "/index.php?option=com_socialads&task=create.cleanup");
										return e.preventDefault();
									}
								...
								// Clear the old values from selections on click of radio button
								techjoomla.jQuery("input[name='geo_type']").click(function () {
									let selectedGeoType = techjoomla.jQuery(this).attr('id');

									if (selectedGeoType === "byregion") {
										techjoomla.jQuery("#region_hidden").attr('value', '');
										techjoomla.jQuery("#selections_region").find("li").remove();
									} else if (selectedGeoType === "bycity") {
										techjoomla.jQuery("#city_hidden").attr('value', '');
										techjoomla.jQuery("#selections_city").find("li").remove();
									} else if (selectedGeoType === "everywhere") {
										techjoomla.jQuery("#region_hidden").attr('value', '');
										techjoomla.jQuery("#city_hidden").attr('value', '');
										techjoomla.jQuery("#selections_region").find("li").remove();
										techjoomla.jQuery("#selections_city").find("li").remove();
								...
								// Validate geo targeting and related inputs
								if (stepId == 'ad-targeting' && data.direction === 'next') {
									let selectedCountry = techjoomla.jQuery("#country_hidden").val();
									let selectedRegion = techjoomla.jQuery("#region_hidden").val();
									let selectedCity = techjoomla.jQuery("#city_hidden").val();
									let selectedGeoType = techjoomla.jQuery("input[name='geo_type']:checked").attr('id');
									let geoTargetingEnabled = techjoomla.jQuery("input[name='geo_targett']:checked").val();

									if (geoTargetingEnabled === "1") {
										if (selectedCountry === "") {
											alert(Joomla.JText._('COM_SOCIALADS_GEO_TARGETING_COUNTRY_REQUIRED'));
											return e.preventDefault();
										}

										if (selectedGeoType === "byregion" && selectedRegion === "") {
											alert(Joomla.JText._('COM_SOCIALADS_GEO_TARGETING_REGION_REQUIRED'));
											return e.preventDefault();
										}

										if (selectedGeoType === "bycity" && selectedCity === "") {
											alert(Joomla.JText._('COM_SOCIALADS_GEO_TARGETING_CITY_REQUIRED'));
											return e.preventDefault();
										}
									}
								}
								...
								if (response['payAndReviewHtml']) {
									...
									// To check first payment gateway by default if only one available
									let totalPaymentGateways = jQuery('input[name ="ad_gateways"]').length;
									if (totalPaymentGateways === 1) {
										jQuery('input[name ="ad_gateways"]')[0].checked = true;
										jQuery('input[name ="ad_gateways"]').attr('checked', 'checked').trigger('change');
									}
								...
								if (response['initialFeeWarning']) {
									alert(response['initialFeeWarning']);
								}
				...
				techjoomla.jQuery('#MyWizard').on('changed', function (e, data) {
					...
					if ((stepthisactive == 'ad-targeting' && allowWholeAdEdit == 0) || (stepthisactive == 'ad-pricing' && unlimited_ad_checked == 1)) {
						...
						if (stepthisactive == 'ad-targeting') 
						{
							if (addMoreCredit == 1 && isCheckedAdMoreCredit == 0) {
							}
							// Ad payment not done so user can edit full ad
		...
		calculateReach: function () {
			...
			techjoomla.jQuery.ajax({
				type: 'POST',
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.calculateReach',
				data: targetfields,
				dataType: 'json',
		...
		cancelCreate: function () {
			if (isAdmin == 1) {
				if (confirm(Joomla.JText._('COM_SOCIALADS_CANCEL_AD')) === true) {
					window.location.assign(Joomla.getOptions('system.paths').base + "/index.php?option=com_socialads&view=forms");
				}
				else {
					return false;
				}
		...
		getUserCampaign: function (userid) {
			jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.getUserCampaigns&userid=' + userid,
				type: 'POST',
				dataType: 'json',
				success: function (data) {
					if (data.length > 1) {
					...
					for (index = 0; index < data.length; ++index) {
						...
						if (option['id'] == camp_id) {
							var op = "<option selected value=" + option['id'] + ">" + option['campaign'] + '</option>';
							techjoomla.jQuery('#ad_campaign').append(op);
							techjoomla.jQuery('#new_campaign').addClass('d-none');
						}
						else {
		...
		changeAdType: function () {
			// if (document.getElementById("adtype").options.length > 1) {
			...
			{
				techjoomla.jQuery.ajax({
					url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.getZones&a_type=' + document.getElementById('adtype').value,
					type: 'GET',
					dataType: "json",
					success: function (data) {
						...
						techjoomla.jQuery("#zone").append('<option disabled="disabled" value="" selected>' + Joomla.JText._('COM_SOCIALADS_AD_SELECT_ZONE') + '</option>');

						if (data != '') {
							for (i = 0; i < data.length; i++) {
								if (parseInt(data[i].zone_id) == parseInt(ad_zone_id)) {
									techjoomla.jQuery("#zone").append('<option value="' + data[i].zone_id + '" selected>' + data[i].zone_name.replace(/<[^>]*>/g, '') + '</option>');
								} else {
									techjoomla.jQuery("#zone").append('<option value="' + data[i].zone_id + '" >' + data[i].zone_name.replace(/<[^>]*>/g, '') + '</option>');
								}
							}
			...
			if (zoneId)
			{
				techjoomla.jQuery.ajax({
					...
					success: function (data) {
						...
						sa.create.countChars('ad_title', 'ad_title_charsText', '{CHAR}' + Joomla.JText._('COM_SOCIALADS_CHAR_REMAINING'), techjoomla.jQuery('#max_tit').val(), techjoomla.jQuery('#ad_title').val(), '');
						if (document.getElementById('adtype').value != 'affiliate')
							sa.create.countChars('body', 'ad_body_charsText', '{CHAR}' + Joomla.JText._('COM_SOCIALADS_CHAR_REMAINING'), techjoomla.jQuery('#max_body').val(), techjoomla.jQuery('#body').val(), '');
						...
						for (i = 0; i < data[0].layout.length; i++) //showing the layout's radio buttons
						{
							...
							if (parseFloat(bootstrap_version) == 5.0) {

								techjoomla.jQuery("#layout1").append('<span class="layout_span col-lg-8 col-md-8 col-sm-12 col-xs-12"><input class="layout_radio" type="radio" name="layout" value="' + data[0].layout[i] + '" ' + sel + ' onclick="sa.create.changeLayout(this.value)" ><img class="layout_radio" src="' + data[0].base + 'plugins/socialadslayout/' + data[0].layout[i] + '/' + data[0].layout[i] + '/layout.png" ></span>');

							}
							else {
								techjoomla.jQuery("#layout1").append('<span class="layout_span span8"><input class="layout_radio" type="radio" name="layout" value="' + data[0].layout[i] + '" ' + sel + ' onclick="sa.create.changeLayout(this.value)" ><img class="layout_radio" src="' + data[0].base + 'plugins/socialadslayout/' + data[0].layout[i] + '/' + data[0].layout[i] + '/layout.png" ></span>');
							}
							//added by sagar
							if (sel == 'checked')
						...
						if (document.getElementById('totalamount') !== null && camp_price == "wallet_mode") {
		...
		changeLayout: function (rad) {
			...
			var zone = document.getElementById('zone').options[document.getElementById('zone').selectedIndex].value.replace(/<[^>]*>/g, '');
			/*added by manoj

				2.7.5 stable*/
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.changeLayout&layout=' + rad + '&title=' + techjoomla.jQuery("#ad_title").val().replace(/<[^>]*>/g, '') + '&body=' + techjoomla.jQuery("#body").val().replace(/<[^>]*>/g, '') + '&img=' + techjoomla.jQuery('#upimg').val() + '&a_type=' + adtype + '&a_zone=' + zone + '&params=' + techjoomla.jQuery('#params').val(),
				type: 'GET',
				dataType: "json",
				success: function (data) {
		...
		countChars: function (entrance, exit, msg, characters, value, event) {
			...
			// Commented this for show max length message for affiliate ad
			if (document.getElementById('adtype').value == 'affiliate') {
				return;
			}
			...
			if (length <= 0) {
				...
				if (document.getElementById('adtype').value == '' || document.getElementById('zone').value == '') {
					alert(Joomla.JText._('COM_SOCIALADS_ENTERING_TITLE_ERROR'))
				}
			} else {
		...
		getPaymentGatewayHtml: function (ele, orderid, payPerAd, loadingMsg, loadingImgPath) {
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=payment.getPaymentGatewayHtml&payPerAd=' + payPerAd + '&gateway=' + ele + '&order_id=' + orderid + '&tmpl=component',
				type: 'POST',
				data: '',
				dataType: 'text',
				beforeSend: function () {
		...
		getStatesList: function (countryId, Dbvalue, selOptionMsg) {
			...
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=checkout.loadState&country=' + country + '&tmpl=component',
				type: 'GET',
				dataType: 'json',
				success: function (data) {
		...
		getPromoterPlugins: function (uid) {
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.getPromoterPlugins&tmpl=component&uid=' + uid,
				type: 'GET',
				dataType: 'json',
				success: function (response) {
		...
		populatePromotePlgList: function () {
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + "/index.php?option=com_sa&task=create.getPromotePluginPreviewData&id=" + (techjoomla.jQuery(".promotplglist").val()),
				type: "GET",
				dataType: "json",
				success: function (result) {
					...
					/*This is needed in case of promote plugins, where ad image is coming from plugin, so we mark this as no-mandatory field*/
					techjoomla.jQuery("#ad_img_label").text(Joomla.JText._('COM_SOCIALADS_PROMOTE_PLUGIN_AD_UPLOAD_MEDIA'));

					if (result.params) {
						techjoomla.jQuery("#params").val(result.params);
					}
		...
		validateCreateForm: function (stepId, unlimited_ad) {
			...
			if (document.getElementById("adtype").value == "text_media" || document.getElementById("adtype").value == "text" || document.getElementById("adtype").value == "media" || document.getElementById("adtype").value == "html5_zip") {
				var theurl = document.adsform.url2.value;
				if (theurl == "") {
					techjoomla.jQuery("#url2").css("border-color", "red");
					message = message + Joomla.JText._('COM_SOCIALADS_URL_VALID') + "\n";
					hasError = 1;
				}

				if (hasError != 1 && !theurl.match(/^[\s]*[A-Za-z0-9]+([\-\.]{1}[A-Za-z0-9]+)*\.[A-Za-z]{2,5}(:[0-9]{1,5})?(\/.*)?[\s]*$/)) {
					techjoomla.jQuery("#url2").css("border-color", "red");
			...
			/*Title validation*/
			if (document.getElementById("adtype").value == "text_media" || document.getElementById("adtype").value == "text" || document.getElementById("adtype").value == "html5_zip" || document.getElementById("adtype").value == "affiliate") {
				if (document.getElementById("ad_title").value == "") {
					message = message + Joomla.JText._('COM_SOCIALADS_TITLE_VALID') + "\n";
					techjoomla.jQuery("#ad_title").css("border-color", "red");
					hasError = 1;
				}
			}
			...
			if (document.getElementById("adtype").value == "text_media" || document.getElementById("adtype").value == "text" || document.getElementById("adtype").value == "affiliate") {
				if (document.getElementById("body").value == "") {
					message = message + Joomla.JText._('COM_SOCIALADS_BODY_VALID') + "\n";
					techjoomla.jQuery("#body").css("border-color", "red");
					hasError = 1;
				}
			}

			if (document.getElementById("adtype").value == "media" || document.getElementById("adtype").value == "text_media" || document.getElementById("adtype").value == "html5_zip") {
				if (document.getElementById("ad_image").value == "" && document.getElementById("upimg").value == "") {
					message = message + Joomla.JText._('COM_SOCIALADS_MEDIA_VALID') + "\n";
					techjoomla.jQuery(".uneditable-input").css("border-color", "red");
					hasError = 1;
				}
			}
		...
		pricingValidations: function (camp_currency_daily, selected_pricing_mode, sa_minimum_charge, sa_minimum_charge_msg, currency, sa_invalid_date_msg, sa_ad_date_need_msg, sa_wrong_dates_msg, sa_invalid_credits_msg, sa_chk_contextual_msg) {
			...
			if (selected_pricing_mode == "wallet_mode") {
				...
				if ((parseInt(chargeoption) >= 2) && (unlimited_ad != 1)) {
					...
					if(!techjoomla.jQuery('#datefrom').hasClass('while-adding-more-days')) {
		...
		walletModeValidation: function (camp_currency_daily, currency) {
			...
			if (!techjoomla.jQuery('#new_campaign').hasClass('d-none')) {
				var camp_name = document.getElementById('camp_name').value;
				var camp_amount = document.getElementById('camp_amount').value;

				if (camp_name == '' && (ncamp == '0' || ncamp == ''))			//both list camp or new camp not present
				{
				...
				if (ncamp == '0' || ncamp == '')		//if new camp is none then check for select list camp
				{
		...
		showNewCamp: function () {
			/*If click on new button reset the value of campaign select box to 0*/
			if (isActive == 0) {
				document.getElementById('ad_campaign').disabled = true;
				document.getElementById('campaignDetails').style.display = 'none';
				techjoomla.jQuery("#ad_campaign").val(techjoomla.jQuery("#ad_campaign option:first").val());
				techjoomla.jQuery('#new_campaign').removeClass('d-none');
				document.getElementById("click").style.display = "none";
				document.getElementById("imps").style.display = "none";
				techjoomla.jQuery('.newCampBtnText').addClass('d-none');
				techjoomla.jQuery('.existCampBtnText').removeClass('d-none');
				isActive = 1;
			}
			else {
				if (techjoomla.jQuery("#ad_campaign > option").length != 0) {
					document.getElementById('ad_campaign').disabled = false;
				}

				techjoomla.jQuery('#new_campaign').addClass('d-none');
				techjoomla.jQuery('.existCampBtnText').addClass('d-none');
				techjoomla.jQuery('.newCampBtnText').removeClass('d-none');
				isActive = 0;
			}
		},
		...
		getZonePricing: function () {
			...
			jQuery('#click_span').html(Joomla.JText._('COM_SOCIALADS_RATE_PER_CLICK') + getFormattedPrice(click_price, currency));
			jQuery('#imps_span').html(Joomla.JText._('COM_SOCIALADS_RATE_PER_IMP') + getFormattedPrice(click_imp, currency));
		...
		applyCoupon: function (ck_val, jtoken) {
			if (typeof (jtoken) === "undefined") {
				jtoken = csrfToken;
			} else {
				csrfToken = jtoken;
			}
			...
			if (techjoomla.jQuery('#sa_coupon_chk').is(':checked')) {
				if (techjoomla.jQuery('#totalamount').val() == '') {
					...
					techjoomla.jQuery.ajax({
						url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=payment.getcoupon&' + jtoken + '=1&coupon_code=' + document.getElementById('sa_coupon_code').value,
						type: 'GET',
						dataType: 'json',
						success: function (data) {
							techjoomla.jQuery('input[name ="sa_cop"]').keyup(function () {
								if (techjoomla.jQuery('#sa_coupon_code').val() == '') {
									techjoomla.jQuery('.sa_cop_details').hide();
								}
							});

							if (data.not_exists) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_EXISTS'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}

							if (data.unpublished) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_UNPUBLISHED'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}

							if (data.expired) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_EXPIRED'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}

							if (data.max_use_exceeded) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_EXCEEDED'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}

							if (data.max_use_per_user_exceeded) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_PER_USER_EXCEEDED'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}

							if (data.not_started) {
								alert(techjoomla.jQuery('#sa_coupon_code').val() + ' ' + Joomla.JText._('COM_SOCIALADS_COP_NOT_YET_ACTIVE'));
								techjoomla.jQuery('#sa_coupon_code').val('');
								return;
							}
							...
							if (amt < 0) {
								alert(Joomla.JText._('COM_SOCIALADS_COP_CODE_CANT_APPLIED'));
							} else {
								techjoomla.jQuery('.sa_cop_details').show();
								techjoomla.jQuery('#sa_cop_price').html(getFormattedPrice(amt, currency));
								techjoomla.jQuery('#sa_cop_afterprice').html(getFormattedPrice(val, currency))
								techjoomla.jQuery('#dis_cop').html(getFormattedPrice(val, currency));
		...
		calculateTotal: function () {
			var chargeoptionsel = 0;
			var click_price = 0;
			var click_date = 0;
			var click_imp = 0;
			var totaldisplay = 0;
			let eleTotalDisplay = document.getElementById('totaldisplay');

			if (document.getElementById('chargeoption') !== null) {
				chargeoptionsel = document.getElementById('chargeoption').value;
			}

			/** global: sa_zone_pricing */
			if (sa_zone_pricing !== 0) {
				click_price = document.getElementById('pric_click').value;
			...
			if (eleTotalDisplay !== null && eleTotalDisplay !== undefined) {
				totaldisplay = eleTotalDisplay.value;

				if (totaldisplay !== "") {
					if (!Number.isInteger(parseInt(totaldisplay))) {
						alert(sa_invalid_credits_msg);
						totaldisplay = eleTotalDisplay.value = "";
						return;
					}

					totaldisplay = eleTotalDisplay.value = parseInt(totaldisplay);
				}
			}
			...
			if (document.getElementById('total_days_label') !== null) {
			...
			if (document.getElementById('chargeoption').value == '1') {
				...
				document.getElementById('priceperclick').style.display = '';
				...
				if (totaldisplay == '') {
					...
					if (initialFee && needToPayInitialFee && !addedInitialFee) {
						amt1 = amt1 + Number(initialFee);
						jQuery('.totalPriceWithPlatformFee').removeClass('d-none');
					} 
	...
	payment: {
		reloadPayment: function() {
			if (techjoomla.jQuery('.make-payment-page').length) {
				if (Number(techjoomla.jQuery('#jform_amount').val()) < Number(min_pre_balance)) {
					alert(Joomla.JText._('COM_SOCIALADS_MINIMUM_AMOUNT_ERROR') + min_pre_balance)
					techjoomla.jQuery('.showNextTab').prop("disabled",true);
					techjoomla.jQuery('.showNextTab').addClass("disabled");
				} else {
					techjoomla.jQuery('.showNextTab').prop("disabled",false);
					techjoomla.jQuery('.showNextTab').removeClass("disabled");
				}
			}
			if(jQuery('input[name=payment_gateway]:checked').length > 0)
			{
				sa.payment.makePayment(document.querySelector('input[name="payment_gateway"]:checked').value);
			}
		},
		/*Function to apply a coupon code*/
		applyCoupon: function (jtoken) {
			...
			if (techjoomla.jQuery('#jform_coupon_code').val() == '') {
				Joomla.renderMessages({
					'error': [Joomla.JText._('COM_SOCIALAD_PAYMENT_ENTER_COUPON_CODE')]
				});
			}
			else {
				techjoomla.jQuery.ajax({
					url: '?option=com_sa&task=payment.getcoupon&' + jtoken + '=1&coupon_code=' + coupon_code,
					type: 'GET',
					dataType: 'json',
					success: function (data) {
						if (data !== null) {
							if (data.not_exists) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_COP_EXISTS')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.unpublished) {
								Joomla.renderMessages({ 'error': [coupon_code + ' ' + Joomla.JText._('COM_SOCIALADS_COP_UNPUBLISHED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.expired) {
								Joomla.renderMessages({ 'error': [coupon_code + Joomla.JText._('COM_SOCIALADS_COP_EXPIRED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.max_use_exceeded) {
								Joomla.renderMessages({ 'error': [coupon_code + ' ' + Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_EXCEEDED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.max_use_per_user_exceeded) {
								Joomla.renderMessages({ 'error': [coupon_code + ' ' + Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_PER_USER_EXCEEDED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.not_started) {
								Joomla.renderMessages({ 'error': [coupon_code + ' ' + Joomla.JText._('COM_SOCIALADS_COP_NOT_YET_ACTIVE')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data[0] && data[0].val_type == 1) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALAD_PAYMENT_COUPON_CODE_IN_PERCENT')] });
								return false;
							}
							else {
								var value = data[0].value;
								amt = sa.payment.round(cal - value);

								if (amt <= 0)
									amt = 0;
								techjoomla.jQuery('#coupon_value').html(getFormattedPrice(value, currency));
								if (techjoomla.jQuery('#coupon_discount').hasClass('hidePaymentFields')) {
									techjoomla.jQuery('#coupon_discount').show();
								} else {
									techjoomla.jQuery('#coupon_discount').removeClass('d-none');
								}
								
								techjoomla.jQuery('#dis_amt').html(getFormattedPrice(amt, currency));

								if (techjoomla.jQuery('#dis_amt1').hasClass('hidePaymentFields')) {
									techjoomla.jQuery('#dis_amt1').show();
								} else {
									techjoomla.jQuery('#dis_amt1').removeClass('d-none');
								}

								if (amt == 0) {
									techjoomla.jQuery('#pay_gateway').hide();
									techjoomla.jQuery('#coupon').html('<input type="button" class="button btn btn-primary" id="add_coupon" value="' + submit + '" onclick="sa.payment.AddCouponPayment(\'' + coupon_code + '\',\'' + cal + '\')">');
									techjoomla.jQuery('#coupon_div').show();
									techjoomla.jQuery('#coupon_div').removeClass('d-none');

								} else {
									techjoomla.jQuery('#pay_gateway').show();
									techjoomla.jQuery('#coupon_div').hide();
									techjoomla.jQuery('#coupon_div').addClass('d-none');
								}
							}
						}
						else {
							Joomla.renderMessages({ 'error': [document.getElementById('jform_coupon_code').value + ' ' + Joomla.JText._('COM_SOCIALADS_PAYMENT_COUPON_NOT_EXISTS')] });
		...
		makePayment: function (pay_method) {
			var cal = document.getElementById('jform_amount').value;
			var minValue = document.getElementById('jform_amount').getAttribute('min');

			var cop_text = document.getElementById('jform_coupon_code').value;

			if (jQuery('input[name="jform[coupon_result]"]').length)
			{
				var cop_dis_opn_hide = jQuery('input[name="jform[coupon_result]"]:checked').val();
			} else {
				var cop_dis_opn_hide = jQuery('input[name="coupon_result"]:checked').val();
			}
			...
			if (cal < minValue) 
			{
				Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_PAYMENT_MIN_AMT_TO_PAY') + minValue] });
			...
			if (cal == '' || isNaN(cal)) 
			{
				Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_PAYMENT_ENTER_CORRECT_AMT')] });
				document.getElementById('jform_amount').focus();
				techjoomla.jQuery('#jform_amount').val('');
				return false;
			}

			if (techjoomla.jQuery('.taxValue').length && techjoomla.jQuery('.taxValue').text())
			{
				tax = techjoomla.jQuery('.taxValue').text();
			}

			techjoomla.jQuery('#html-container').show();
			techjoomla.jQuery.ajax({
				url: '?option=com_sa&task=payment.makePayment&processor=' + pay_method + '&amount=' + cal + '&cop=' + document.getElementById('jform_coupon_code').value + '&cop_dis_opn_hide=' + cop_dis_opn_hide + '&tax=' + tax,
				type: 'GET',
				dataType: 'html',
				beforeSend: function () {
		...
		showCoupon: function (rad, mim_bal) {
			...
			if (amt == '' || isNaN(amt)) {
				...
				Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALAD_PAYMENT_AMOUNT_NEEDS_TO_PAY') + mim_bal] });
				document.getElementById('jform_amount').focus();
				techjoomla.jQuery('input[name=jform\\[coupon_result\\]]:checked').val([0]);
				return false;
			}
			...
			if (rad == 1) {
				...
				if (techjoomla.jQuery('#coupon_discount').hasClass('hidePaymentFields')) {
					techjoomla.jQuery('#coupon_discount').hide();
				} else {
					techjoomla.jQuery('#coupon_discount').addClass('d-none');
				}
				techjoomla.jQuery("#couponHide").hide();
				if (techjoomla.jQuery('#dis_amt1').hasClass('hidePaymentFields')) {
					techjoomla.jQuery('#dis_amt1').hide();
				} else {
					techjoomla.jQuery('#dis_amt1').addClass('d-none');
				}
			}
		},
		...
		AddCouponPayment: function (coupon_code, value) {
			techjoomla.jQuery.ajax({
				...
				success: function (data) {
					if (data != 0) {
						window.location = Joomla.getOptions('system.paths').base + '/index.php?option=com_socialads&view=wallet';
					}
				}
			});
		}
	...
	wallet: {
		...
		applyCouponCode: function (jtoken) {
			var coupon_code = document.getElementById('coupon_code').value;

			if (jQuery('#coupon_code').val() == '') {
				Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALAD_PAYMENT_ENTER_COUPON_CODE')] });
			} else {
				jQuery.ajax({
					url: '?option=com_sa&task=payment.getcoupon&' + jtoken + '=1&coupon_code=' + coupon_code,
					type: 'GET',
					...
					success: function (data) {
						if (data !== null) {
							if (data.not_exists) {
								Joomla.renderMessages({ 'error': [coupon_code + ' ' + Joomla.JText._('COM_SOCIALADS_COP_EXISTS')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}
							...
							if (data.expired) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_COP_EXPIRED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.max_use_exceeded) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_EXCEEDED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.max_use_per_user_exceeded) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_COP_MAX_USE_PER_USER_EXCEEDED')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}

							if (data.not_started) {
								Joomla.renderMessages({ 'error': [Joomla.JText._('COM_SOCIALADS_COP_NOT_YET_ACTIVE')] });
								techjoomla.jQuery('#coupon_code').val('');
								return;
							}
							...
							Joomla.renderMessages({ 'error': [document.getElementById('coupon_code').value + ' ' + Joomla.JText._('COM_SOCIALADS_PAYMENT_COUPON_NOT_EXISTS')] });
							document.getElementById('coupon_code').value = '';
						}
					}
				});
			...
			jQuery.ajax({
				...
				success: function (data) {
					if (data !== null) {
						...
						alert(Joomla.JText._('COM_SOCIALADS_WALLET_COUPON_NOT_ADDED'));
					}
...
var saAdmin = {
	...
	ads: {
		selectAdStatus: function (appid, ele) {
			...
			if (status == 2) {
				var reply = saAdmin.ads.validatePrompt();
			}
			if (!(reply == null)) {
		...
		selectZoneIfNotExists: function (appid, ele) {
			...
			Joomla.submitform('forms.updatezone');
			return;
		},
		...
		submitButtonAction: function (task) {
			if (task == 'forms.delete') {
				...
				if (r === false) {
					...
					var r = confirm(Joomla.JText._('COM_SOCIALADS_ADS_AD_ORDER_DELETE_CONFIRM'));
					if (r === false) {
						return false;
					}
				}
	...
	adorders: {
		selectStatusOrder: function (appid, ele) {
			if (document.getElementById('pstatus0').value == "RF") {
				var r = confirm(Joomla.JText._('COM_SOCIALADS_ADS_STATUS_REFUND_PROMPT_BOX'));
				if (r === false) {
					location.reload();
					return false;
				}
			...
			if (document.getElementById('pstatus0').value == "E") {
				var r = confirm(Joomla.JText._('COM_SOCIALADS_ADS_STATUS_CANCEL_PROMPT_BOX'));
				if (r === false) {
					location.reload();
					return false;
				}
			...
			Joomla.submitform('adorders.save');
			return;
		}
	},
	...
	coupon: {
		...
		checkMaxUses: function () {
			var maxUses = document.getElementById('jform_max_use').value;
			var maxPerUses = document.getElementById('jform_max_per_user').value;

			// "Maximum uses" of coupon should be more than "maximum uses per user"
			if(maxPerUses && maxUses) {
				if (maxUses < maxPerUses) {
					alert(Joomla.JText._('COM_SOCIALADS_MAX_USES_ERROR_MSG'));
					return 0;
				}
			}
		...
		submitButtonAction: function (task) {
			if (task == 'coupon.apply') {
				...
				var checkMaxUse = saAdmin.coupon.checkMaxUses();
				if (checkDate === 0 || checkMaxUse === 0) {
					return false;
				}
	...
	dashboard: {
		...
		drawDonutChart: function (ajaxRespData) {
			/*Check if any orders*/
			if (ajaxRespData[0]['porders'] == 0 && ajaxRespData[0]['corders'] == 0 && ajaxRespData[0]['rorders'] == 0 && ajaxRespData[0]['cancelled_orders'] == 0) {
				return;
			}
			/*Hide no data msg*/
			...
			Morris.Donut({
				...
				data: [{
					...
					label: Joomla.JText._('COM_SOCIALADS_CANCELLED_ORDERS'),
					value: ajaxRespData[0]['cancelled_orders']
				}],
				colors: ["#fefe00", "#5cb85c", "#ff0000", "#f0ad4e"]
			});
		},
				...
				success: function (data) {
					if (!data) {
						...
						techjoomla.jQuery.each(data, function (i, feeds) {
							...
							newsFeedsHtml += '<div class="col-sm-4 col-md-5 col-lg-5 ">';
							...
							newsFeedsHtml += '</time>';
							newsFeedsHtml += '</div>';
							newsFeedsHtml += '<div class="col-sm-7 col-md-6 col-lg-6">';
							newsFeedsHtml += '<div>';
	...
	campaign: {
		...
		campaignSubmitButton: function (task) {
			if (task == 'campaign.cancel') {
				...
				{
					alert(Joomla.JText._('COM_SOCIALADS_START_DATE_MUST_BE_LESS_FROM_END_DATE'));
					return false;
				}
	...
	zone: {
		zoneAdTypes: function (value) {
			if (value == "text") {
				...
				techjoomla.jQuery('#img_width_ratio').hide();
				techjoomla.jQuery('#img_height_ratio').hide();
				...
				/*Remove required attribute for media fields*/
				techjoomla.jQuery('#jform_img_height_ratio').removeAttr("required");
				techjoomla.jQuery('#jform_img_width_ratio').removeAttr("required");
				/*Remove required class for media fields*/
				techjoomla.jQuery('#jform_img_height_ratio').removeClass("required");
				techjoomla.jQuery('#jform_img_width_ratio').removeClass("required");
			...
			} else if (value == "media" || value == 'html5_zip') {
				techjoomla.jQuery('#img_width').show();
				techjoomla.jQuery('#img_height').show();
				techjoomla.jQuery('#img_width_ratio').show();
				techjoomla.jQuery('#img_height_ratio').show();
				...
				techjoomla.jQuery('#jform_img_height_ratio').attr("required", true);
				techjoomla.jQuery('#jform_img_width_ratio').attr("required", true);
		...
		validateFields: function (task) {
			if (task == 'zone.cancel') {
				...
				if (task != 'zone.cancel' && document.formvalidator.isValid(document.getElementById('adminForm'))) {
					var form = document.adminForm;
					...
					if (addtype == "media") {
						...
						var value = jQuery('input[name="jform[use_image_ratio]"]:checked').val();
						if (value == '0') {
							...
							if (saAdmin.zone.trim(form.jform_img_width_ratio.value) == "") {
								document.getElementById("validate_img_width_ratio").innerHTML = Joomla.JText._("COM_SOCIALADS_YOU_PROVIDE_A_IMG_WIDTH_RATIO");
								flag = 1;
								return false;
							} else {
								if (isNaN(saAdmin.zone.trim(form.jform_img_width_ratio.value)) || (parseInt(form.jform_img_width_ratio.value) == 0)) {
									document.getElementById("validate_img_width_ratio").innerHTML = Joomla.JText._("COM_SOCIALADS_VALIDATE_NON_ZERO_NUMERIC");
									flag = 1;
									return false;
								}
							}
							if (saAdmin.zone.trim(form.jform_img_height_ratio.value) == "") {
								document.getElementById("validate_img_height_ratio").innerHTML = Joomla.JText._("COM_SOCIALADS_YOU_PROVIDE_A_IMG_HEIGHT_RATIO");
								flag = 1;
								return false;
							} else {
								if (isNaN(saAdmin.zone.trim(form.jform_img_height_ratio.value)) || (parseInt(form.jform_img_height_ratio.value) == 0)) {
		...
		useImageRatioToggle: function (elem) {
			var adtypeValue = jQuery('#jform_ad_type').val();
			if (adtypeValue != 'text')
			{
				var value = jQuery('input[name="jform[use_image_ratio]"]:checked').val();
				if (value == '1') {
...
{
	currencyDisplayFormat    = sa_json_config.currency_display_format;

	currencyDisplayFormat = currencyDisplayFormat.replace('{AMOUNT}', " " + price);
	currencyDisplayFormat = currencyDisplayFormat.replace('{CURRENCY_SYMBOL}', " " + currency);

	return currencyDisplayFormat;
}

function showCampaignDetails(selectedId) {
    var detailsDiv = document.getElementById('campaignDetails');
    detailsDiv.style.display = 'block';
	document.getElementById('ad_campaign').value;
    detailsDiv.innerHTML = '';

    if (selectedId && campaigns) {
        var selectedCampaign = campaigns.find(campaign => campaign.id == selectedId);

        if (selectedCampaign) {
            const startDate = selectedCampaign.start_date ? selectedCampaign.start_date : " - ";
            const endDate = selectedCampaign.end_date ? selectedCampaign.end_date : " - ";

            const backendCampInfo = document.getElementById('backendCampInfo');

            detailsDiv.innerHTML =
				`<div class="campaign-info campaign-details ${backendCampInfo ? 'ml-18' : ''}">
					<p><strong>${Joomla.JText._('COM_SOCIALADS_DAILY_BUDGET')}</strong> ${selectedCampaign.daily_budget}</p>
					<p><strong>${Joomla.JText._('COM_SOCIALADS_START_DATE')}</strong> ${startDate}</p>
					<p><strong>${Joomla.JText._('COM_SOCIALADS_END_DATE')}</strong> ${endDate}</p>
				</div>`;
        } else {
            detailsDiv.innerHTML = `<p class="text-danger">${Joomla.JText._('COM_SOCIALADS_DEFAULT_MSG')}</p>`;
        }
    } else {
        detailsDiv.innerHTML = `<p class="ml-18 text-danger">${Joomla.JText._('COM_SOCIALADS_DEFAULT_MSG')}</p>`;
    }
}

function checkSuperUser()
{
	var user = techjoomla.jQuery('#adsform #ad_creator_id_id').val();
	if(user)
	{
		techjoomla.jQuery.ajax({
			url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=create.getUserType&user=' + user + '&tmpl=component',
			/*url: '?option=com_socialads&controller=buildad&task=autoSave&stepId=' + stepId + '&tmpl=component&format=raw&sa_sentApproveMail=' + sa_sentApproveMail,*/
			type: 'POST',
			async: false,
			data: {user : user},
			dataType: 'json',
			success: function (response) {
				if(response['hasUnlimitedAdPermssion'] == 1)
				{
					techjoomla.jQuery('#adsform .unlimitedAdrow').removeClass('d-none');
				}
				else
				{
					techjoomla.jQuery('#adsform .unlimitedAdrow').addClass('d-none');
				}

				if(response['hasAffiliateAdPermssion'] == 1)
				{
					techjoomla.jQuery('#adsform #altadbutton').prop( "disabled", false );
					if (techjoomla.jQuery("#adsform #adtype option[value='affiliate']").length > 0)
					{
						techjoomla.jQuery('#adsform #adtype').val('');
					}
					document.querySelectorAll("#adsform #adtype option").forEach(opt => {
						if (opt.value == "affiliate") {
							opt.disabled = false;
						}
					});
				}
				...
				{
					techjoomla.jQuery('#adsform #altadbutton').prop( "disabled", true );
					techjoomla.jQuery('#adsform #altadbutton').prop( "checked", false );
					document.querySelectorAll("#adsform #adtype option").forEach(opt => {
						if (opt.value == "affiliate") {
							opt.disabled = true;
						}
...
techjoomla.jQuery( document ).ready(function() {
	checkSuperUser();
	techjoomla.jQuery('#adsform #ad_creator_id_id').attr('onchange', 'checkSuperUser()');

	if (techjoomla.jQuery('#sa-payment #amount').length)
	{
		techjoomla.jQuery('#sa-payment #amount').change( function() {
			if (techjoomla.jQuery('#sa-payment [name="ad_gateways"]:checked').length)
			{
				var value = techjoomla.jQuery('#sa-payment [name="ad_gateways"]:checked').val();
				techjoomla.jQuery('#sa-payment input[name="ad_gateways"][value="' + value + '"]').trigger('click');;
