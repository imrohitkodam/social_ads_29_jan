techjoomla.jQuery(document).ready(function() {
	...
	if(selected_pricing_mode == 'wallet_mode' && document.getElementById('ad_campaign').length == 0)
	{
		document.getElementById('ad_campaign').disabled = true;
		jQuery('#no_campaign').html(Joomla.JText._('COM_SOCIALAD_CAMPAIGN_ARE_UNPUBLISHED'));
	}
	else if (selected_pricing_mode == 'wallet_mode')
	{
		sa.create.hideNewCampaign();
	}
...
function hidePayment(selector) {
	...
	if(selector == 1) {
		...
		techjoomla.jQuery("#ad_campaign").attr("disabled", "disabled");
		...
		techjoomla.jQuery("#ad_campaign").removeAttr("disabled");
