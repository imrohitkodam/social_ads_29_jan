 ...
 * @since  1.6
 */
class SocialadsModelDashboard extends ListModel
{
	/**
	...
	public function getActiveAdCount()
	{
		...
		$query->where($db->quoteName('ad.created_by') . ' = ' . $user->id);
		$query->where($db->quoteName('ad.state') . ' = ' . 1);
		$db->setQuery($query);
	...
	public function getAllOrdersIncome()
	{
		...
		$query->where($db->quoteName('comment') . ' != '. $db->q('AUTO_GENERATED'));
	...
	public function getTopAds()
	{
		...
		$query->order($db->quoteName('ctr') . ' DESC');
		$query->setLimit('5');
		$db->setQuery($query);
	...
	public function getPendingOrders($paymentmode)
	{
		...
		elseif ($paymentmode == "wallet_mode")
		{
			...
			$query1->select($db->quoteName(array('a.id', 'a.amount')));
			$query1->from($db->quoteName('#__ad_orders', 'a'));
			...
			$query1->where($db->quoteName('a.status') . " = " . $db->q('P'));
			$query->setLimit('5');
			$db->setQuery($query1);
			$pendingorders = $db->loadObjectList();
		}
