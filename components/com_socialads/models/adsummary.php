...
class SocialadsModelAdsummary extends ListModel
{
	...
	public function getstatsforpiechart()
	{
		...
		$query->from($db->qn('#__ad_stats', 's'));
		$query->where($db->qn('s.display_type') . ' = ' . 0);
		$query->where($db->qn('s.ad_id') . ' = ' . $adid);
		...
		$query1->from($db->qn('#__ad_archive_stats', 'aas'));
		$query1->where($db->qn('impression') . ' != ' . 0);
		$query1->where($db->qn('aas.ad_id') . ' = ' . $adid);
		...
		$query2->from($db->qn('#__ad_stats', 's'));
		$query2->where($db->qn('s.display_type') . ' = ' . 1);
		$query2->where($db->qn('s.ad_id') . ' = ' . $adid);
		...
		$query3->from($db->qn('#__ad_archive_stats', 'aas'));
		$query3->where($db->qn('impression') . ' != ' . 0);
		$query3->where($db->qn('aas.ad_id') . ' = ' . $adid);
	...
	public function getstatsforlinechart()
	{
		...
		$query->from($db->qn('#__ad_stats'));
		$query->where($db->qn('ad_id') . ' = ' . $ad_id);
		$query->where("DATE(time) BETWEEN DATE('" . $from_date . "') AND DATE('" . $to_date . "')");
		...
		$query1->from($db->qn('#__ad_archive_stats'));
		$query1->where($db->qn('ad_id') . ' = ' . $ad_id);
		$query1->where("DATE(date) BETWEEN DATE('" . $from_date . "') AND DATE('" . $to_date . "')");
