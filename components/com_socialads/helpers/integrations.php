...
class SaIntegrationsHelper
{
		...
		if (!empty($eschk))
		{
			for ($i = 0; $i < count($fields); $i++)
			{
				if ($fields[$i]->mapping_fieldtype != 'textbox')
				{
					...
					$field_array = new stdClass;
					$field_array->id = $fields[$i]->mapping_fieldid;
					...
					if (!empty($options_value))
					{
						...
						foreach ($options_value as $option_value_title)
						{
							// EasySocial default field value are in the Object and Custom field value are in Array
							if (is_object($option_value_title))
							{
								$options[] = $option_value_title->title;
							}
							else
							{
								$options[] = $option_value_title;
							}
						}

						$options = implode("\n", $options);
						$field_array->options = $options;
						$field_option[] = $field_array;
	...
	{
		if (File::exists(JPATH_SITE . "/components/com_comprofiler/plugin/language/default_language/language.php"))
		{
			global $_CB_framework, $_CB_database, $ueConfig, $mainframe;

			if (defined('JPATH_ADMINISTRATOR'))
			{
				if (!File::exists(JPATH_ADMINISTRATOR . '/components/com_comprofiler/plugin.foundation.php'))
				{
					return false;
				}
			...
			{
				if (!File::exists($mainframe->getCfg('absolute_path') . '/administrator/components/com_comprofiler/plugin.foundation.php'))
				{
					return false;
				}
	...
	public static function DisplayjlikeButton($ad_url,$id,$title)
	{
		...
		$grt_response = Factory::getApplication()->triggerEvent('onAfterSaAdDispay', array('com_socialads.viewad', $jlikeparams));
