...
class SaAdEngineHelper
{
	...
	public static $ad_entry_number = 1;
	...
	public function __construct($engineType = 'local', $userid = 0, $extra = 0)
	{
		...
		{
			self::$_my = Factory::getUser($userid);
		}

		self::$_fromemail = $extra;

		self::$_params = ComponentHelper::getParams('com_socialads');
	...
	public static function getAdsForZone($params, $moduleId)
	{
		...
		if (!is_array($params))
		{
			if (is_object($params) && ($params->get('popup_ad', 0) || $params->get('floater_ad', 0)))
			{
				$number      = 1;
			}
		}
		...
		foreach ($resultads_session_ads[$zone_id] as $key => $ad)
		{
			if (!empty($ad))
			{
				...
				if (empty($ad->seen))
				{
					...
					$adForDisplay = true;
					$statue_adcharge = self::getAdStatus($ad->ad_id, $adForDisplay);
	...
	public static function getAdsByPriority($params)
	{
		...
		$func_list2 = array('Guest', 'Affiliate');
		shuffle($func_list2);
		$func_list = array_merge($func_list, $func_list2);
		array_push($func_list, 'Alt');
			...
			if ($remain != 0 || $remain == '')
			{
				...
				if (!empty($session_ads))
				{
					foreach ($session_ads as $zone_id => $value)
					{
						...
						$remain_num = self::pushinSlot($zone_id, $params, $value, $remain);
	 ...
	 * @param   integer  $no_of_ads  No. of ads
	 *
	 * @return  integer
	 */
	public static function pushInSlot($zone_id, $params, $ads, $no_of_ads = '')
	{
		...
		if (count(self::$_resultads[$zone_id]))
		{
			...
			if (is_array($ads))
			{
				$ad_ids = array_udiff($ads, self::$_resultads[$zone_id], array(self::class, 'compareIds'));
			}*/
	...
	public static function getQueryJoinCampaigns()
	{
		...
		if (self::$_params->get('payment_mode') == 'wallet_mode')
		{
			...
			$join = " LEFT JOIN #__ad_campaign as c ON c.id = a.camp_id  ";
		}
	...
	public static function getQueryWhereCommon($params, $function_name)
	{
		...
		if (self::$_params->get('payment_mode') == 'wallet_mode' && $function_name != "alt" && $function_name != "affiliate")
		{
			...
			$comon_query[] = " ((c.state = 1 OR a.ad_noexpiry=1) AND (
		...
		require_once JPATH_SITE . '/libraries/tjmobiledetect/lib_tjmobiledetect.php';
		$detect = new TjMobieDetect;

		if ($detect->isMobile())
		{
			$comon_query[] = " (a.display_ad_on LIKE '%mobile%' OR a.display_ad_on = '') ";
		}
		else 
		{
			$comon_query[] = " (a.display_ad_on LIKE '%desktop%' OR a.display_ad_on = '') ";
		}
	 ...
	 * @return  [type]         [description]
	 */
	public static function getAdStatus($adid, $adForDisplay = false)
	{
		if (!$adid)
		{
			return false;
		...
		*/
		...
		if (self::$_params->get('payment_mode') == 'wallet_mode' && $ad_alt_exp->ad_noexpiry == 0
			&& $ad_alt_exp->ad_alternative == 0 && $ad_alt_exp->ad_affiliate == 0)
		{
			// $status_ads = self::check_balance($adid, $ad_charge);

			$status_ads = SaCreditsHelper::checkBalance($adid, $ad_charge);
		}
		elseif (self::$_params->get('payment_mode') == 'pay_per_ad_mode' && $ad_alt_exp->ad_noexpiry == 0
			&& $ad_alt_exp->ad_alternative == 0 && $ad_alt_exp->ad_affiliate == 0)
		{
			$status_ads = SaCreditsHelper::checkCreditsAvailable($adid);
		}
		else
		{
			$status_ads = 1;
	...
	public static function getAdHtml($addata, $adseen = 0, $adrotation = 0, $widget = '')
	{
		...
		if ($adseen == 1)
		{
			if ($addata == 0)
			{
				...
				$addata->ad_image = str_replace(Uri::base(), '', $buildadsession->get('upimg'));
		...
		$tit              = $addata->ad_title;
		if ($addata->layout == 'layout7')
		{
			$addata->ad_title = mb_substr($tit, 0, 30, 'UTF-8');
		}
		else
		{
		...
		$module = ModuleHelper::getModule('mod_socialads');

		/* if (($mainframe->isClient("site")) && !empty($addata))*/
		if ($mainframe->isClient("site") && $adseen == 1 && !empty($addata))
		{
		...
		@$html = '<div class = "text-break ad_prev_main' . $mod_sfx . ' ' . $class . '" preview_for= "' .
		...
		$plugin = $addata->layout;

		HTMLHelper::stylesheet('media/com_sa/css/helper.min.css', $options);
		...
		if (File::exists($layout))
		{
			...
			$css       = 'plugins/socialadslayout/' . $plugin . '/' .
				$plugin . '/';
			$css      .= $debugMode ? 'layout.css' : 'layout.min.css';
			HTMLHelper::stylesheet($css, $options);
		...
		else
		{
			...
			if ($addata->ad_image != '')
			{
				...
				$html .= '<img class="ad_prev_img"  src="' . Uri::root() . $addata->ad_image . '" border="0" />';
	...
	public static function getAdHtmlByMedia($upload_area, $ad_image, $ad_body, $ad_link, $ad_layout, $adzone, $track = 0, $ad_id = '')
	{
		...
		else
		{
			...
			$fpath           = Uri::root() . $ad_image;
			$fextension      = File::getExt($fpath);
			$ad_type         = $media->get_ad_type($fextension);

			if (strpos($ad_image, 'html5_zip') !== false)
			{
				$ad_type    = 'zip';
			}
			...
			*/
			...
			{
				case "image":
					$saLayout = new FileLayout('adhtml_image', $rootPath = JPATH_ROOT . '/layouts/adhtml');
				...
				case "flash":
					...
					$saLayout = new FileLayout('adhtml_flash', $rootPath = JPATH_ROOT . '/layouts/adhtml');
				...
				case "video":
					...
					$saLayout = new FileLayout('adhtml_video', $rootPath = JPATH_ROOT . '/layouts/adhtml');
				...
				case "zip":
					...
					$saLayout = new FileLayout('adhtml_html5_zip', $rootPath = JPATH_ROOT . '/layouts/adhtml');
