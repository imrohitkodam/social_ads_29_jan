...
class SocialadsPaymentHelper
{
	...
	public function adddays($ad_id,$no_days)
	{
		...
		if ($adDetails[0]->ad_enddate == "0000-00-00" || is_null($adDetails[0]->ad_enddate))
		{
			$endDate = Factory::getDate($adDetails[0]->ad_startdate, 'UTC');
			$endDate->modify(" +" . $no_days . " day");
		}
		else
		{
			$date     = $adDetails[0]->ad_enddate;
			$endDate = Factory::getDate($date, 'UTC');
		...
		$addata->ad_enddate = $endDate->toSQL();;
	 ...
	 * @param   string   $payPerAd  ad creation mode
	 *
	 * @return  void
	 *
	 * @since  1.6
	...
	public function getInvoiceDetail($order_id,$pg_nm,$payPerAd= "pay_per_ad_mode")
	{
		...
		{
			$orderstatus = Text::_('COM_SOCIALADS_INVOICE_STATUS_COMPLEATE');
		}
		elseif ($orderDetails->status == 'P')
		{
			$orderstatus = Text::_('COM_SOCIALADS_INVOICE_STATUS_PENDING');
		}
		elseif ($orderDetails->status == 'E')
		{
			$orderstatus = Text::_('COM_SOCIALADS_INVOICE_STATUS_COMPLEATE_ERROR_OCCURED');
		}
		else
		{
			$orderstatus = Text::_('COM_SOCIALADS_INVOICE_AMOUNT_CANCELLED');
		}
		...
		targe='_blank'>" . Text::_("COM_SOCIALADS_EMAIL_THIS_LINK") . "</a>";
		...
		$user = Factory::getUser($orderDetails->payee_id);
		...
		{
			$approve_msg = Text::_("COM_SOCIALADS_INVOICE_MAIL_ADMIN_NOT_APPROVED");

			if ($orderDetails->status == 'C')
			{
				$approve_msg = Text::_("COM_SOCIALADS_INVOICE_MAIL_ADMIN_APPROVED");
			}

			// FOR ADVERTISER INVOICE AND ORDER CONFIRM MAIL
			$advertiserEmailBody = Text::_("COM_SOCIALADS_INVOICE_MAIL_CONTENT");

			// NOW find & REPLACE TAG
			$find = array('[SEND_TO_NAME]','[ADVERTISER_NAME]','[SITENAME]','[SITELINK]','[ADTITLE]','[ORDERID]','[ADMIN_APPROVAL_MSG]');
			$replace = array($adUserName, $adUserName, $siteName, $sitelink, $adTitle, $displayOrderid, $approve_msg);
			...
			$subject = Text::sprintf('COM_SOCIALADS_INVOICE_MAIL_SUB', $displayOrderid);
			$status  = SaCommonHelper::sendmail($recipient, $subject, $advertiserEmailBody, '', 0, "");

			// -------------- ADMIN INVOICE MAIL  COPY --------------------
			$adminEmailBody = Text::_("COM_SOCIALADS_INVOICE_MAIL_CONTENT_ADMIN_COPY");
			$orderPrice = SaCommonHelper::getFormattedPrice($orderDetails->amount,  $params->get('currency'));
			$admin_approve_msg = '';
			$admin_approve_msg = '';

			if ($orderDetails->status == 'C')
			{
				$admin_approve_msg = Text::sprintf("COM_SOCIALADS_APPRVE_MAIL_TO_ADMIN_ADD_MSG", $manageAdLink);
			}
			...
			$subject = Text::sprintf('COM_SOCIALADS_INVOICE_MAIL_ADVERTISER_ADMIN_SUBJECT', $displayOrderid);
			$status  = SaCommonHelper::sendmail($frommail, $subject, $adminEmailBody, '', 0, "");
		}
		else
		{
			// ADVERTISER MAIL
			$advertiserEmailBody = Text::_("COM_SOCIALADS_WALLET_ADDED_BALACE_ADVETISER_EMAIL");
			...
			$advertiserEmailBody = str_replace($find, $replace, $advertiserEmailBody);
			$advertiserEmailBody = $advertiserEmailBody . "<br> <br>" . $inv_adver_Html;
			$subject = Text::sprintf('COM_SOCIALADS_WALLET_ADDED_BALACE_ADVETISER_EMAIL_SUBJECT', $displayOrderid);
			$status = SaCommonHelper::sendmail($recipient, $subject, $advertiserEmailBody, '', 0, "");
	...
	public function userInfo($order_id)
	{
		...
		$query->where($db->quoteName('ao.id') . ' = ' . $order_id);
	...
	public function getcoupon($c_code = '')
	{
		...
		if ($c_code)
		{
			$query = "SELECT value, val_type
			 FROM #__ad_coupon
			 WHERE ((NOW() BETWEEN from_date AND exp_date)
				OR ((from_date <= NOW() OR exp_date is null) AND (exp_date is null OR CAST(exp_date AS CHAR(20)) = '0000-00-00 00:00:00') )
				OR ((exp_date >= NOW() OR from_date is null) AND (from_date is null OR CAST(from_date AS CHAR(20)) = '0000-00-00 00:00:00') )
			)
			 AND (
				 max_use  > (
	...
	public function getOrderAndAdDetail($order_id, $syncDetail=0)
	{
		...
		$db = Factory::getDBO();
		$query         = $db->getQuery(true);
		$query->select($db->quoteName('b.ad_id'));
		$query->from($db->quoteName('#__ad_payment_info', 'b'));
		$query->where($db->quoteName('b.order_id') . ' = ' . $order_id);
		...
		if (!empty($adid))
		{
			...
			*/
			$query = $db->getQuery(true);
			$query->select('a.*');
			$query->select($db->qn(array('o.original_amount', 'o.prefix_oid', 'o.amount', 'o.status', 
						'o.coupon', 'o.tax', 'o.processor', 'pi.ad_credits_qty', 'o.extras')));
			$query->from($db->quoteName('#__ad_data', 'a'));
			$query->join('LEFT', $db->quoteName('#__ad_payment_info', 'pi') . 'ON' . $db->quoteName('pi.ad_id') . '=' . $db->quoteName('a.ad_id'));
			$query->join('LEFT', $db->quoteName('#__ad_orders', 'o') . 'ON' . $db->quoteName('o.id') . '=' . $db->quoteName('pi.order_id'));
			$query->where($db->qn('o.id') . ' = ' . $order_id);
		}
		else
		{
			// Add wallet mode
	...
	public function syncOrderDetail($order_id)
	{
		$db = Factory::getDBO();
		$query = $db->getQuery(true);
		$query->select($db->qn(array('a.original_amount', 'a.coupon', 'a.tax', 'a.tax_details')));
		$query->from($db->quoteName('#__ad_orders', 'a'));
		$query->where($db->qn('a.id') . ' = ' . $order_id);
		$query->where($db->qn('a.id') . ' != ' . $db->q('C'));
	...
	public function getSlabDetails($duration)
	{
		...
		{
			if ($slab && is_array($slab) && $slab['duration'] == $duration)
			{
				return $slab;
			}
		}
	...
	public function migrateads_camp($migrate_need_check)
	{
		...
		$query->from($db->qn('#__ad_data'));
		$query->group($db->qn('created_by'));
		$db->setQuery($query);
		$created_by = $db->loadColumn();

		// Get all approved ads count
		$query1 = $db->getQuery(true);
		$query1->select('COUNT(ad_approved) as ad_approved');
		$query1->from($db->qn('#__ad_data'));
		$query1->where($db->qn('ad_approved') . ' = ' . 1);
		$db->setQuery($query1);
		$ad_approved = $db->loadColumn();

		if ($ad_approved[0] > 0)
		{
			// $json = 1; old code Not sure for this But the migrate message display all the time
			$json = 0;
		}
		else
		{
		...
		{
			$query = $db->getQuery(true);
			$query->select($db->qn(array('ad_id', 'ad_zone', 'ad_credits_balance', 'ad_payment_type', 'ad_noexpiry')));
			$query->from($db->qn('#__ad_data'));
			$query->where($db->qn('created_by') . ' = ' . $creator_id);
			$query->where($db->qn('camp_id') . ' = ' . 0);
			$db->setQuery($query);
			$ads_info = $db->loadobjectlist();

			// Create new camp as old camp.....
			$query = $db->getQuery(true);
			$query->select($db->qn(array('campaign', 'id')));
			$query->from($db->qn('#__ad_campaign'));
			$query->where($db->qn('campaign') . ' = ' . $db->q('Old Ads for user ' . $creator_id));
			$query->where($db->qn('created_by') . ' = ' . $creator_id);
			$db->setQuery($query);
				...
				foreach ($ads_info as $ad_info)
				{
					...
					if ($ad_info->ad_credits_balance)
					{
						...
						if ($sa_params->get('zone_pricing', 0) == 1)
						{
							...
							{
								$query = $db->getQuery(true);
								$query->select($db->qn(array('per_imp', 'per_click')));
								$query->from($db->qn('#__ad_zone'));
								$query->where($db->qn('id') . ' = ' . $ad_info->ad_zone);
								$db->setQuery($query);
						...
						$insertpay->ip_address = TJGeoLocationHelper::getUserIP();
	...
	public function migrateads_old($migrate_need_check)
	{
		...
		foreach ($created_by as $creator_id)
		{
			...
			$query->where($db->qn('created_by') . ' = ' . $creator_id);
			$query->where($db->qn('camp_id') . ' != ' . 0);
			$query->where($db->qn('ad_alternative') . ' = ' . 0);
			$query->where($db->qn('ad_noexpiry') . ' = ' . 0);
			$query->where($db->qn('ad_affiliate') . ' = ' . 0);
			$db->setQuery($query);
			$ads_info = $db->loadobjectlist();

			// Get balance
			$query = $db->getQuery(true);
			$query->select($db->qn(array('id', 'balance')));
			$query->from($db->qn('#__ad_wallet_transc'));
			$query->where($db->qn("time") . " = (select MAX(time) from #__ad_wallet_transc where user_id =" . $creator_id . ")");
			$query->where($db->qn("user_id") . " = " . $creator_id);
			$db->setQuery($query);
			...
			$query = $db->getQuery(true);
			$query->select($db->qn('processor'));
			$query->from($db->qn('#__ad_orders'));
			$query->where($db->qn('payee_id') . ' = ' . $creator_id);
			$db->setQuery($query);
					...
					if (isset($current_bal[0]->balance))
					{
						...
						if ($current_bal[0]->balance)
						{
							...
							*/
							...
							$comment_array = array();
							$comment_array[] = 'COM_SOCIALADS_WALLET_SPENT_DONE_FROM_MIGRATION';
							$comment_array[] = $ad_info->ad_id;
							$comment = implode('|', $comment_array);
