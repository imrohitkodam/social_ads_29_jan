...
class SaAdsHelperContext extends SaAdsHelper
{
	...
	public static function getAdTargetData($params, $adType, $engineType = 'local')
	{
		...
		if (isset($context_target_param) && isset($searchPages))
		{
		...
		if (!empty($smartSearch) && empty($searchkeywords))
		{
			$vv = Uri::getInstance()->toString();
			$vv = str_replace(Uri::base() . '', '', $vv);
			$vv = str_replace(Uri::base() . '', '', $vv);
			...
			$querylink	= $db->getQuery(true);
			$querylink->select('DISTINCT(link_id)');
			$querylink->from($db->quoteName('#__finder_links'));
			$querylink->where($db->quoteName('url') . ' = ' . $db->q($searchSrting));
			$querylink->orWhere($db->quoteName('route') . ' = ' . $db->q($searchSrting));
			...
			if (!empty($link_id))
			{
				...
				$query->select($db->quoteName(array('term', 'weight')));
				$query->from($db->quoteName('#__ad_contextual_terms'));
				$query->where($db->qn('link_id') . ' IN (' . implode(',', $link_id) . ')');
				$query->where($db->quoteName('term') . ' != ' . $db->q(''));
				$query->order($db->quoteName('weight') . ' DESC');
				$query->setLimit('100');
		...
		if (empty($meatKeyword) && empty($searchkeywords))
		{
	...
	public static function getContextSearchData()
	{
		...
		// If Contextual Ads For Search Pages are enabled & search page urls are set
		if (isset($context_target_param) && $context_target_param && isset($searchPages))
		{
