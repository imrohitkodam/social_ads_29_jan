...
$lang->load('plg_socialadspromote_jsprofile', JPATH_ADMINISTRATOR);
 ...
 * @since  1.6
 */
class PlgSocialadsPromoteJsProfile extends CMSPlugin
{
	/**
	...
	public function onSocialAdPromoteData($id)
	{
		...
		if (!empty($jschk))
		{
			...
			if ($fieldid)
			{
				...
				$query->join(
					'LEFT', $db->quoteName('#__community_fields_values', 'cfv') . 'ON' . $db->quoteName('cu.userid') . '=' . $db->quoteName('cfv.user_id')
					);
