 ...
 * @since  1.6
 */
class PlgsocialadstargetingJsprofiletargeting extends CMSPlugin
{
	/**
	...
	public function __construct($subject, $config)
	{
		...
		{
			$this->_plugin = PluginHelper::getPlugin('socialadstargeting', 'jsprofiletargeting');
	...
	public function _getLayoutPath($plugin, $group, $layout = 'default')
	{
		...
		$defaultPath = JPATH_SITE . '/plugins/' . $group . '/' . $plugin . '/' . $plugin . '/' . $layout . '.php';
		$templatePath = JPATH_SITE . '/templates/' . $app->getTemplate() . '/html/plugins/' .
		$group . '/' . $plugin . '/' . $plugin . '/' . $layout . '.php';
