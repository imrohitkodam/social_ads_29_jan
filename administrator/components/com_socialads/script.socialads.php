...
class Com_SocialadsInstallerScript
{
	...
	private $removeFilesAndFolders = array(
		'files'	=> array(
			'components/com_socialads/controller/registration.php',
			'components/com_socialads/model/registration.php',
		),
		'folders' => array(
		...
		'components/com_socialads/classes/geolocation',
		'components/com_socialads/views/registration',
		)
	);
	...
	public function getpluginparams($group,$api,$paramstr)
	{
		...
		if (JVERSION >= 1.6)
		{
			$plugin = PluginHelper::getPlugin($group, $api);
	...
	public function postflight($type, $parent)
	{
		...
		$this->setDefaultACLPermissions($type);
		$this->enableCLIPlugin($type);
	}
	...
	public function setDefaultLayout($type)
	{
		if ($type == 'install' && JVERSION >= '4.0.0')
		{
			...
			$query->where($db->quoteName('element') . ' = ' . $db->quote('com_socialads'));
			$db->setQuery($query);
			$data = $db->loadObject();
	 ...
	 * @since 4.0.0
	 */
	public function setDefaultACLPermissions($type)
	{
		if ($type == 'install')
		{
			$db = Factory::getDbo();
			...
			$fields = array(
				$db->quoteName('rules') . ' = ' . $db->quote('{"core.create":{"2":1}}')
			);
			
			$conditions = array(
				$db->quoteName('name') . ' = ' . $db->quote('com_socialads'),
				$db->quoteName('title') . ' = ' . $db->quote('com_socialads')
			);
	...
	/**
	 * Set default boACL permissions
	 * Enable the CLI plugin for j4x
	 *
	 * @param   string  $type  install, update or discover_update
	 * 
	 * @return void
	 ...
	 * @since 4.0.0
	 */
	public function enableCLIPlugin($type)
	{
		if ($type == 'install' && JVERSION >= '4.0.0')
		{
			$db = Factory::getDbo();
			$pluginElements  = ['statsemail', 'archivestats'];
			$fields = array(
			...
			$conditions = array(
				...
				$db->quoteName('element') . ' IN (' . implode(',', $db->quote($pluginElements)) .  ')',
			);
		...
		$uninstall_queue = array(
			'plugins' => array(
				'socialadspromote' => array(
					'plug_promote_cb' => 0,
					'plug_promote_esprofile' => 0,
					'plug_promote_jsevents' => 0,
					'plug_promote_jsprofile' => 0,
					'plug_promote_sobi' => 0,
					'plug_promote_jsgroups' => 0
				)
			)
		);
	 ...
	 *
	 * @param   JInstaller  $parent           Component installation path
	 * @param   Mixed       $uninstall_queue  uninstall_queue
	 *
	...
	public function fix_db_on_update()
	{
		...
		$columns = array
		(
			array("old" => "zone_type","new" => "orientation",
				"type" => " TINYINT(2)  NOT NULL COMMENT 'Orientation for a specific zone Horizontal or Vertical'"),
	 ...
	 *
	 * @param   array  $removeFilesAndFolders  removeFilesAndFolders
	 *
	 * @return  void
	 */
	...
	public function changePluginParams()
	{
		...
		$condition = $db->quoteName('element') . ' = "layout1"';
		...
		$fields = $db->quoteName('params') . "='" . $data . "'";
		$condition = $db->quoteName('element') . ' = "layout3"';
		$query->update($db->quoteName('#__extensions'))->set($fields)->where($condition);
		$db->setquery($query);
		$db->execute();
		...
		$fields = $db->quoteName('params') . "='" . $data . "'";
		$condition = $db->quoteName('element') . ' = "layout5"';
		$query->update($db->quoteName('#__extensions'))->set($fields)->where($condition);
		$db->setquery($query);
		$db->execute();
	}
	...
	/**
	 * Function to add ad specific user details in SocialAds
	 *
	 * @return  void
	 *
	 * @since  3.1.13
	 */
	public function adSpecificUserData()
	{
		// Get all SA users
		$db    = Factory::getDbo();
		$query = $db->getQuery(true);
		...
		{
			// For given users get ALL ads
			$query = $db->getQuery(true);
			$query->select($db->quoteName('ad_id'));
			$query->from($db->quoteName('#__ad_data'));
			$query->where($db->quoteName('created_by') . '=' . $user->user_id);
			$query->where($db->quoteName('ad_alternative') . '= 0');
			$query->where($db->quoteName('ad_noexpiry') . '= 0');
			$db->setQuery($query);
			$ads = $db->loadColumn();

			foreach ($ads as $ad)
			{
				// Check if the ad ID already exist in users table
				$check = $this->checkUserDataExist($user->user_id, $ad);
				...
				// If Users data don't have ad ID
				elseif (empty($user->ad_id))
				{
					$db = Factory::getDbo();
					$query = $db->getQuery(true);
					$fields = array($db->quoteName('ad_id') . ' = ' . (int) $ad);
					$conditions = array($db->quoteName('user_id') . ' = ' . (int) $user->user_id);

					$query->update($db->quoteName('#__ad_users'))->set($fields)->where($conditions);

					$db->setQuery($query);

					$result = $db->execute();
				}
				// IF ad have no billing information then add entry
				elseif ($user->ad_id != $ad)
				{
					$userObject = $user;
					...
					if (!$db->insertObject('#__ad_users', $userObject, 'id'))
					{
						return 0;
					}
				}
	...
	/**
	 * Function to check user data exist or not
	 *
	 ...
	 * @since  3.1
	 */
	public function checkUserDataExist($userId, $adId)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('id');
