 ...
 * @since  1.6
 */
class SocialadsModelDashboard extends BaseDatabaseModel
{
	/**
	 * Constructor.
	 *
	...
	public function getPeriodicOrders($paymentmode)
	{
		...
		if ($paymentmode == 'wallet_mode')
		{
			...
			$query1->select('
				...
				COUNT(IF(status LIKE "RF", 1, NULL)) as rorders,
				COUNT(IF(status LIKE "E",  1, NULL)) as cancelled_orders
			');
	...
	public function getAverageCtr()
	{
		...
		{
			$averagectr = ($averagectr['click'] / $averagectr['impression']) * 100;
		}
	...
	public function getTopAds()
	{
		...
		$query->select('ad_id, ad_title, ((clicks / impressions) * 100) as ctr');
		$query->from($db->quoteName('#__ad_data'));
	...
	public function getNewsFeeds()
	{
		...
		if ($feeds)
		{
			...
			for ($i = 0; $i < 4; $i++)
			{
				...
				$parsedFeeds[$i]->text = OutputFilter::stripImages($parsedFeeds[$i]->text);
				$parsedFeeds[$i]->text = strip_tags($parsedFeeds[$i]->text);
				$parsedFeeds[$i]->text = HTMLHelper::_('string.truncate', $parsedFeeds[$i]->text, 125);
	 ...
	 * Akeeba Admin Tools, (c) 2010-2019 Nicholas K. Dionysopoulos / Akeeba Ltd
	 *
	 * Do we have the Techjoomla Maxmind Plugin installed?
	 *
	 * @return  boolean  False = not installed, True = installed
	 *
	 * @since 3.2.0
	 */
	public function hasTJMaxmindPlugin()
	{
		$db    = Factory::getDbo();
		$query = $db->getQuery(true);
		...
		$query->select('COUNT(*)')
			...
			->where($db->qn('element') . ' = ' . $db->q('tjmaxmind'))
