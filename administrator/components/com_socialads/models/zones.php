 ...
 * @since  1.6
 */
class SocialadsModelZones extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ad_zone', 'a'));
	...
	public function getZoneModules()
	{
		...
		$query->select($db->quoteName('params'))
			...
			->where($db->quoteName('module') . ' LIKE ' . $db->quote('%mod_socialads%'));
	...
	public function getZoneaddatacount($selzoneid)
	{
		...
		if ($selected == 'pay_per_ad_mode')
		{
			$query->select('COUNT(d.ad_id)')
				->from($db->quoteName('#__ad_data', 'd'))
				->join('LEFT', $db->quoteName('#__ad_zone', 'z') . ' ON ' . $db->quoteName('d.ad_zone') . ' = ' . $db->quoteName('z.id'))
				->where($db->quoteName('z.id') . ' = ' . $selzoneid)
				->where($db->quoteName('d.state') . ' = ' . 1)
				->where('(' . $db->quoteName('d.ad_credits_balance') . ' > 0 OR ' . $db->quoteName('d.ad_noexpiry') . ' = 1 OR ' . $db->quoteName('d.ad_alternative') . ' = 1)')
				->where($db->quoteName('d.ad_approved') . ' = ' . 1);
		...
		{
			$query->select('COUNT(DISTINCT d.ad_id)')
				->from($db->quoteName('#__ad_data', 'd'))
				->join('LEFT', $db->quoteName('#__ad_zone', 'z') . ' ON ' . $db->quoteName('d.ad_zone') . ' = ' . $db->quoteName('z.id'))
				->where($db->quoteName('z.id') . ' = ' . $selzoneid)
				->where($db->quoteName('d.state') . ' = ' . 1)
				->where('((SELECT balance FROM #__ad_wallet_transc wt WHERE wt.user_id = d.created_by ORDER BY wt.id DESC LIMIT 1) > 0 OR ' . $db->quoteName('d.ad_noexpiry') . ' = 1 OR ' . $db->quoteName('d.ad_alternative') . ' = 1)')
				->where($db->quoteName('d.ad_approved') . ' = ' . 1);

			$db->setQuery($query);
			$adCount = $db->loadResult();
