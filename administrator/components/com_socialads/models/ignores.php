 ...
 * @since  1.6
 */
class SocialadsModelIgnores extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	public function getListQuery()
	{
		...
		$query->select($db->quoteName('a.ad_feedback'));
		$query->select($db->quoteName('a.idate'));
		$query->from($db->quoteName('#__ad_ignore', 'a'));

		// Join over the user field 'irnored_by'
		$query->join('RIGHT', $db->quoteName('#__users', 'u') . 'ON' . $db->quoteName('u.id') . '=' . $db->quoteName('a.userid'));
		$query->where($db->quoteName('a.adid'). ' = ' . (int) $adid);
