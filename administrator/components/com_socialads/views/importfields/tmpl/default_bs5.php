	...
	<form action="" class="form-validate" method="post" name="adminForm" onSubmit="return saAdmin.importfields.formValidation(this);">
		...
		{?>
			<div class="alert alert-info">
				<span ><?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_JS_IS_NOTINSTALL'); ?> </span>
			</div>
		...
		elseif(empty($this->fields)  && ($integration == 'Community Builder'))
		{
				...
				{?>
					<div class="alert alert-info">
						<span>
							<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_CB_IS_NOTINSTALL'); ?>
						</span>
					</div>
				<?php
				}
				...
				{?>
					<div class="alert alert-info">
						<span>
							<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_CBINSTALL_FIELDS'); ?>
						</span>
					</div>
				<?php
				}
		...
		elseif (empty($this->fields)  && ($integration == 'EasySocial'))
		{
			...
			{?>
				<div class="alert alert-info">
					<span >
						<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_ES_IS_NOTINSTALL'); ?>
					</span>
				</div>
			<?php
			}
			...
			{?>
				<div class="alert alert-info">
					<span>
						<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_ESINSTALL_FIELDS'); ?>
					</span>
				</div>
			<?php
			}
		...
		{?>
			<div class="alert alert-info">
				<span>
					<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_NO_SOCIAL_TAR'); ?>
				</span>
			</div>
		<?php
		}
			...
			<div id='no-more-tables'>
				<table class="table table-responsive" id="List">
					<thead>
						<tr>
							<th>
								<?php echo HTMLHelper::tooltip(Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_TOOLTIP'), Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_HEAD'),  '', Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL'));?>
							</th>
							<th>
								<?php echo HTMLHelper::tooltip(Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_FOR_ADVERTISER_TOOLTIP'), Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_FOR_ADVERTISER_HEAD'),  '', Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_FOR_ADVERTISER'));?>
							</th>
							<th>
								<?php echo HTMLHelper::tooltip(Text::_('COM_SOCIALADS_SOCIAL_TARGETING_MAP_WITH_FIELD_TYPE_TOOLTIP'), Text::_('COM_SOCIALADS_SOCIAL_TARGETING_MAP_WITH_FIELD_TYPE_HEAD'),  '', Text::_('COM_SOCIALADS_SOCIAL_TARGETING_MAP_WITH_FIELD_TYPE'));?>
							</th>
							<th>
							<?php echo HTMLHelper::tooltip(Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FUZZY_OR_EXACT_MATCH_TOOLTIP'), Text::_('COM_SOCIALADS_SOCIAL_TARGETING_SELECT_FUZZY_OR_EXACT_HEAD'),  '', Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FUZZY_OR_EXACT_MATCH'));?>
							</th>
						</tr>
					</thead>
					<tbody>
						...
						foreach ($this->fields as $row)
						{
							...
							<tr class="<?php echo 'row'.$k; ?>" id="<?php echo 'row'.$count; ?>">
								...
								{
									$row->field_label = Text::_("$row->field_label");
								}
								?>
								<td id="<?php echo 'row'.$count.'[1]'; ?>" data-title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL');?>">
									<?php echo $row->field_label . " (" . $row->id . ")"; ?>
								</td>
								<td id="<?php echo 'row'.$count.'[2]'; ?>" data-title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FIELD_LABEL_FOR_ADVERTISER');?>">
									<?php
									$mapval = Text::_("$row->mapping_label") ? Text::_("$row->mapping_label") : Text::_("$row->field_label");
									?>
									<input type="text" class = "input-medium" name="mappinglist[<?php echo $row->id; ?>][label]" <?php echo $disabled; ?> value="<?php echo $mapval; ?>" />
								</td>
								<td style="display:none;" id="<?php echo 'row'.$count.'[3]'; ?>" data-title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_MAP_WITH_FIELD_TYPE');?>">
									<?php echo HTMLHelper::_('select.genericlist', $this->fields, 'mappinglist['.$row->id.'][fieldid]',
									'class="input-medium inputbox" '.$disabled, 'id', 'field_label', $row->id ); ?>
								</td>
								<td id="<?php echo 'row'.$count.'[4]'; ?>" data-title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_MAP_WITH_FIELD_TYPE');?>">
									<?php
									...
									echo HTMLHelper::_('select.genericlist', $list, 'mappinglist['.$row->id.'][fieldtype]', 'class="input-medium form-select inputbox fieldlist " onchange=saAdmin.importfields.numericRangeCheck(this,'.$row->id.','.$count.'); ' .$disabled, 'value', 'text', $row->mapping_fieldtype); ?>
								</td>
								<?php
								$match = array(0=>Text::_("COM_SOCIALADS_SOCIAL_TARGETING_FUZZY"), 1=>Text::_("COM_SOCIALADS_SOCIAL_TARGETING_EXACT"));
								$field_array = array();
								...
								if (!empty($match))
								{
									...
									<td id="<?php echo 'row'.$count.'[5]'; ?>" data-title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_FUZZY_OR_EXACT_MATCH');?>">
										<span id="<?php echo 'row'.$count.'[5]radios'; ?>" style="display:block">
											...
											{
												echo Text::_("COM_SOCIALADS_SOCIAL_TARGETING_EXACT");
											...
											elseif ($row->type=='text' || $row->type=='textbox')
											{
												...
												{
													echo $radiolist = HTMLHelper::_('select.radiolist', $options, 'match['.$row->id.']', 'class="inputbox fieldlist"' .$disabled,
													'value', 'text', $row->mapping_match);
												}
												else
												{
													echo Text::_("COM_SOCIALADS_SOCIAL_TARGETING_DOES_NOT_AAPLY");
												}
											}
											else
											{
						...
						foreach ($this->pluginresult as $rowplugin)
						{
							$countbutton = 0;
							$currentversion = '';
							$element = str_replace('plug_', '' ,$rowplugin->element);

							$pluginXml = JPATH_SITE."/plugins/socialadstargeting/$element/$element.xml";
							...
							<tr class="<?php echo 'row'.$k; ?>">
								...
								<td>
									<span title="<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_PLUGINS');?>" >
								...
								<td colspan="3">
									...
									<div class="sa-imprtfields-controls-inline">
										<span name = "chk<?php echo $element; ?>" id="chk<?php echo $element; ?>"
										...
										if ($countbutton == 1)
										{?>
											<input class="sa-imprtfields-controls-left-margin btn btn-sm" type = "button" onclick = "saAdmin.importfields.installTargetingPlugins(this);" name = "<?php echo $element;?>" id = "<?php echo $element;?>" value = "<?php echo Text::_('COM_SOCIALADS_SOCIAL_TARGETING_PLGINSTALL_CLK');?>" >
										<?php
										} ?>
										<span id="message1<?php echo $element;?>"  ></span>
									</div>
								</td>
							</tr>
							<?php
