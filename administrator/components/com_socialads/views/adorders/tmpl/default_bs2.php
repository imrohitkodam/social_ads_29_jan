				...
				<div id="filter-bar" class="btn-toolbar">
					...
					<div class="btn-group pull-right hidden-phone">
						<?php
							...
							echo HTMLHelper::_('select.genericlist', $this->ostatus, "filter.status", 'class="input-medium" size="1" onchange="document.adminForm.submit();" name="filter_status"', "value", "text", $this->state->get('filter.status'));
							?>
					</div>

					<div class="btn-group pull-right hidden-phone">
						<?php
							echo HTMLHelper::_('select.genericlist', $this->gatewayoptions, "filter_gatewaylist", 'class="ad-status inputbox input-medium" size="1" onchange="document.adminForm.submit();" name="gatewaylist"', "value", "text", $this->state->get('filter.gatewaylist'));
						?>
					</div>
				</div>
								...
								<tr class="row<?php echo $i % 2; ?> <?php echo $row_color; ?>">
									...
									<?php if (isset($this->items[0]->id)): ?>
										<td data-title="<?php echo Text::_('JGRID_HEADING_ID');?>">
											<?php
											$link = Route::_('index.php?option=com_socialads&view=adorders&tmpl=component&layout=details&id=' . $item->id);
											...
											{?>
												...
												<a data-toggle="modal" data-target="#orderDetails<?php echo $item->id ?>Modal">
									...
									<td data-title="<?php echo Text::_('COM_SOCIALADS_ADORDERS_STATUS');?>">
										...
										{
											echo HTMLHelper::_('select.genericlist',$validStatus,'pstatus'.$k,'class="pad_status sa-w-100"  onChange="saAdmin.adorders.selectStatusOrder(' . $item->id . ',this);"',"value","text",$item->status);
										}
										else
										{
											echo $whichever ;
