...
<div class="<?php echo SA_WRAPPER_CLASS;?> sa-ad-order">
	...
	?>
		<div id="j-main-container" class="col-md-12">
			<form action="<?php echo Route::_('index.php?option=com_socialads&view=orders'); ?>" method="post" name="adminForm" id="adminForm">
				<div id="filter-bar" class="btn-toolbar">
					<div class="col-md-12 mt-2">
						<div class="col-md-6 col-sm-12">
							<div class="filter-search btn-group float-start">
								<input type="text" name="filter_search" id="filter_search" class="form-control" placeholder="<?php echo Text::_('COM_SOCIALADS_AD_ORDERS_FILTER_SEARCH'); ?>" value="<?php echo $this->escape($this->state->get('filter_search')); ?>" title="<?php echo Text::_('COM_SOCIALADS_AD_ORDERS_FILTER_SEARCH'); ?>" />
						...
						<?php
						if (JVERSION >= '3.0') : ?>
						<div class="btn-group float-end float-end col-12 col-sm-12 col-md-2 col-lg-1 hidden-phone">
						...
						<div class="btn-group float-end hidden-phone me-2">
							<?php echo HTMLHelper::_('select.genericlist', $this->gatewayoptions, "filter_gatewaylist", 'class="ad-status inputbox input-medium form-select" size="1" onchange="document.adminForm.submit();" name="gatewaylist"', "value", "text", $this->state->get('filter.gatewaylist')); ?>
						</div>
						<?php
						endif; ?>
					</div>
				...
				<?php
					...
					<div id='no-more-tables'>
						<table class="table table-responsive mt-2" id="List">
							<thead>
								<tr>
									<?php
							...
							<tr class="row<?php echo $i % 2; ?> <?php echo $row_color; ?>">
								...
								<td class="tj-width-10" data-title="<?php echo Text::_('COM_SOCIALADS_ORDERS_STATUS');?>">
									<?php
										...
										{
											echo HTMLHelper::_('select.genericlist',$validStatus,'pstatus'.$k,'class="pad_status form-select"  onChange="saAdmin.orders.selectStatusOrder(' . $item->id . ',this);"',"value","text",$item->status);
										}
										else
										{
											echo $whichever ;
